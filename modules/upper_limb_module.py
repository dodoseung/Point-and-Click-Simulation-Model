...
1) Simulating mouse rotation
2) Signal-dependent motor noise

Reference:
1) "The use of ballistic movement as an additional method to assess performance of computer mice"
...
def mouse_noise(vel_x, vel_y, h_pos_x, h_pos_y, forearm, mouseGain, interval):
    dx = (vel_x[1:] + vel_x[:-1]) / 2 * interval
    dy = (vel_y[1:] + vel_y[:-1]) / 2 * interval
    ...
    for idx in range(len(dx)):
        if dx[idx] != 0 or dy[idx] != 0:
            ...
            h_pos_x += hand_dx
            h_pos_y += hand_dy
            mouse_dx, mouse_dy = mouse.get_cursor_displacement(hand_pos_prev, np.array((h_pos_x, h_pos_y)), forearm, mouseGain[idx])
            dx[idx] = mouse_dx
            dy[idx] = mouse_dy
...
def motor_noise(vel_x, vel_y, time, mean_x, mean_y, nc, interval):
    for i in range(1, time + 1):
        v = np.array([vel_x[i], vel_y[i]])
        if np.linalg.norm(v) == 0: vel_dir = np.array([0, 0])
        else: vel_dir = v / np.linalg.norm(v)
    ...
    pos_x_delta = (vel_x[1:] + vel_x[:-1]) / 2 * interval
    pos_y_delta = (vel_y[1:] + vel_y[:-1]) / 2 * interval
